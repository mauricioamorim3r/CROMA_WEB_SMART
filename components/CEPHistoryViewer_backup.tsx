...
const CEPHistoryViewer: React.FC<CEPHistoryViewerProps> = ({ isOpen, onClose }) => {
  ...
  const loadHistoricalData = useCallback(() => {
    try {
      ...
      console.log('🔍 CEP Storage Key:', CEP_STORAGE_KEY);
      console.log('🔍 Dados brutos do localStorage:', stored ? 'ENCONTRADOS' : 'VAZIOS');
      
      const rawData = stored ? JSON.parse(stored) : [];
      console.log('🔍 Total de amostras encontradas:', rawData.length);
      
      // ORDENAR POR DATA - MAIS RECENTE PRIMEIRO (como no Excel)
      const sortedData = rawData.sort((a: CEPHistoricalSample, b: CEPHistoricalSample) => 
        new Date(b.date).getTime() - new Date(a.date).getTime()
      );
      
      console.log('🔍 Dados após ordenação:', sortedData.length);
      ...
      if (sortedData.length > 0) {
        ...
        console.log('⚠️ Nenhum dado histórico encontrado no localStorage');
      }
    } catch (error) {
  ...
  const calculateStatistics = useCallback((componentName: string) => {
    // EXTRAIR VALORES VÁLIDOS DAS ÚLTIMAS 8 AMOSTRAS (dados já ordenados por data)
...
;
    ...
    // Calcular amplitudes móveis (diferença absoluta entre valores consecutivos)
    ...
    // FÓRMULAS EXATAS DO EXCEL
    const controlFactor = 3 * (mobileRangeMean / D2_FACTOR); // 3 * (MR̄ / 1.128)
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full bg-white table-with-relief">
                <thead className="table-header-blue">
                  <tr>
                    <th className="px-4 py-3 text-xs font-bold tracking-wider text-center text-white uppercase border-r border-blue-400/50">Boletim</th>
                    <th className="px-4 py-3 text-xs font-bold tracking-wider text-center text-white uppercase border-r border-blue-400/50">Data</th>
                    ...
                    <th className="px-4 py-3 text-xs font-bold tracking-wider text-center text-white uppercase border-r border-blue-400/50">Status CEP</th>
