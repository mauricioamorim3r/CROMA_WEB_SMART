...
const ComponentTable: React.FC<ComponentTableProps> = ({ components, onComponentChange }) => {
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {components.map((comp, index) => (
              <tr key={comp.id}>
                ...
                <td className={`${TABLE_TD_CLASS} text-center`}>{comp.aga8Min.toFixed(2)}</td>
                <td className={`${TABLE_TD_CLASS} text-center`}>{comp.aga8Max.toFixed(2)}</td>
                <td className={TABLE_TD_CLASS}>
                  <input
                    type="number"
                    step="any"
          ...
          <tfoot>
            <tr className="bg-gray-50">
              <td colSpan={2} className={`${TABLE_TD_CLASS} font-bold text-right`}>Total Molar:</td>
              <td className={`${TABLE_TD_CLASS} font-bold text-center`}>{totalMolarPercent.toFixed(4)} %</td>
              <td colSpan={7} className={TABLE_TD_CLASS}></td>
            </tr>
          </tfoot>
        </table>
      </div>
